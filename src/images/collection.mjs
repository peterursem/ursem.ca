...
export class Collection {
    ...
    constructor(collectionData) {
        ...
        this.numFavs = 4; //collectionData.numFavs;
        this.gridItems = [...Array(this.images.length)];
        this.columns = [];
    ...
    createExpansionToggle() {
        ...
        expandToggle.setAttribute('style', "width: 80vw; margin-top: 50px; margin-inline: 10vw;"); 
        ...
        return new Promise((res, rej) => {
            ...
            if (expanded) {
                for (var img = 0; img < this.images.length; img++) {
                    if(this.gridItems[img] != null) {
                        this.columns[imageNumber++ % 4].appendChild(this.gridItems[img]);
                    }
                    else if(this.images[img].tags.indexOf("hide") == -1) {
                        this.columns[imageNumber++ % 4].appendChild(this.makeGridItem(img).gridElem);
                    }
                }
            }
            //If the grid is not expanded and the "fav" images fit in the given number.
                ...
                this.favs.forEach(fav => {
                    if(this.gridItems[fav] != null)
                        this.columns[imageNumber++ % 4].appendChild(this.gridItems[fav]);
                    else
                        this.columns[imageNumber++ % 4].appendChild(this.makeGridItem(fav).gridElem);
                });
            }
